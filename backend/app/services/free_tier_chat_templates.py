    ...
    if selected_skills:
        if len(selected_skills) == 1:
            ...
            usage_steps = [
                ...
                "执行前先查看规划状态与安全标记；若出现 manual_review 或 excluded，请先做人工复核。",
                "客户端执行时会根据返回的 Skill 信息去官方插件中心或 Skill 中心抓取依赖，并在本地组合。",
                "若 OpenClaw 运行面已安装 clawhub CLI，只有当用户明确提出下载或安装指令时，才应由当前客户端按 slug 自行处理。",
                "如果可以直接执行，请明确回复“确认执行”。",
            ]
            ...
            usage_steps = [
                ...
                "执行前先查看每个 Skill 的 planning 状态与安全标记；若出现 manual_review 或 excluded，请先做人工复核。",
                "客户端执行时会根据返回的 Skill 信息去官方插件中心或 Skill 中心抓取依赖，并在本地组合。",
                "若 OpenClaw 运行面已安装 clawhub CLI，只有当用户明确提出下载或安装指令时，才应由当前客户端按 slug 自行处理。",
                "如果可以直接执行，请明确回复“确认执行”。",
            ]
        ...
        usage_steps = [
            ...
            "如果继续执行，请先人工确认通用流程是否适合当前数据源和权限范围。",
